...
```shell
git clone https://github.com/ontum/frappe_docker_ontum.git
cd frappe_docker
```
...
```shell
bench new-site dev-app.localhost --no-mariadb-socket
```

The same command can be run non-interactively as well:

```shell
bench new-site dev-app.localhost --mariadb-root-password 123 --admin-password admin --no-mariadb-socket
```

The command will ask the MariaDB root password. The default root password is `123`.
This will create a new site and a `dev-app.localhost` directory under `frappe-bench/sites`.
...
```shell
bench new-site mypgsql.local --db-type postgres --db-host postgresql
```
...
```shell
bench --site dev-app.localhost set-config developer_mode 1
bench --site dev-app.localhost clear-cache
```
...
Your website will now be accessible at location [dev-app.localhost:8000](http://dev-app.localhost:8000)
...
```shell
bench --site dev-app.localhost console
```
...
Replace `dev-app.localhost` with your site and run the following code in a Jupyter cell:

```python
import frappe
frappe.init(site='dev-app.localhost', sites_path='/workspace/development/frappe-bench/sites')
frappe.connect()
